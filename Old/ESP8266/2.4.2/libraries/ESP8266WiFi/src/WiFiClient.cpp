...
{
  Serial << "WiFiClient.cpp(114): WiFiClient()::connect(): Begin" << endl;
    ip_addr_t addr;
    addr.addr = ip;

    if (_client) {
      Serial << "WiFiClient.cpp(119): WiFiClient()::connect(): _client is NULL" << endl;
        stop();
        _client->unref();
        _client = nullptr;
    }
    ...
    tcp_pcb* pcb = tcp_new();
    if (!pcb) {
      Serial << "WiFiClient.cpp(138): WiFiClient()::connect(): pcb is NULL" << endl;
       return 0;
    }
    ...
    if (res == 0) {
        _client->unref();
        _client = nullptr;
        Serial << "WiFiClient.cpp(152): WiFiClient()::connect(): res == 0" << endl;
        return 0;
    }

...
// [CGB]
void AdafruitIO::init(const char *user, const char *key)
{
  _username = user;
  _key = key;
  _init();	
...
// [CGB]
void AdafruitIO::connect(const char *ssid, const char *pass)
{
  if(_err_sub) {
    // setup error sub
    _err_sub = new Adafruit_MQTT_Subscribe(_mqtt, _err_topic);
...
AdafruitIO_Feed* AdafruitIO::feed(const char* name, bool Init) // [CGB]
{
  return new AdafruitIO_Feed(this, name, Init); // [CGB]
}

...
class AdafruitIO {
  ...
  public:
	...
	void init(const char *user, const char *key);	// [CGB]
    void connect();
	void connect(const char *ssid, const char *pass); //[CGB]
	
    void run(uint16_t busywait_ms = 0);

    AdafruitIO_Feed* feed(const char *name, bool Init=true);
  ...
  protected:
	...
	virtual void _connect(const char *ssid, const char *pass) = 0; // [CGB]

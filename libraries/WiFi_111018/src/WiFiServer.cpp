...
void WiFiServer::begin(){
  Serial << "WiFiServer::begin(): Begin" << endl;
  if(_listening){
    Serial << "WiFiServer::begin(): return() because _listening is TRUE" << endl;
    return;
  }
  struct sockaddr_in server;
  Serial << "WiFiServer::begin(): Call socket(AF_INET , SOCK_STREAM, 0)" << endl;
  sockfd = socket(AF_INET , SOCK_STREAM, 0);
  if (sockfd < 0){
    Serial << "WiFiServer::begin(): return() because sockfd < 0" << endl;
    return;
  }
  server.sin_family = AF_INET;
  server.sin_addr.s_addr = INADDR_ANY;
  server.sin_port = htons(_port);
  Serial << "WiFiServer::begin(): Call bind()" << endl;
  if(bind(sockfd, (struct sockaddr *)&server, sizeof(server)) < 0){
    Serial << "WiFiServer::begin(): return() because bind() failed" << endl;
    return;
  }
  if(listen(sockfd , _max_clients) < 0){
    Serial << "WiFiServer::begin(): return() because listen() failed" << endl;
    return;
  }
  ...
  _noDelay = false;
  _accepted_sockfd = -1;
  Serial << "WiFiServer::begin(): Done" << endl;
  return;
} //begin

...
!
!      ! --> transform the page:
!      GW_ADD_LOADING_IMG (mypage, "local.gif") % display image when page loads
...
!      :PARAM            :VALUES
!      :color            :a|b|c|d|e|f|g (progressbar|button depending on theme)
...
IF !GW_SILENT_LOAD
  ...
  GW_LPG$="<html><head><meta charset='ISO-8859-2'><meta name='viewport' content='width=device-width,initial-scale=1,minimum-scale=1,maximum-scale=1,user-scalable=no'><style>html *{font-family:Roboto !important} p{color:#764fbe} progress{width:80%} #content1{position:absolute;margin:auto;top:0;right:0;bottom:0;left:0;width:250px;height:150px}</style><script>function RFO(data){Android.dataLink(data);}</script></head><body><div id='page1'><div id='content1'><script>window.onload=function(){RFO('ready');}</script><div style='margin:0 auto;text-align:center;'><div><p id='label1'><b>Loading the <img height='14' src='"+GW_LOGO$+"' /> lib...</b><br/>&nbsp;</p><progress id='slider1' min='0' max='100' step='0.1' value='0'></progress></div><div><p></p></div><p><i>www.rfo-basic.com</i></p></div></div></div></body></html>"
  ...
  WHILE i
    ...
    i=IS_IN("FN.END\n", gw$, i+1)
  REPEAT
ELSE % IF GW_SILENT_LOAD
  GW_LPG$="<html><head><meta charset='ISO-8859-2'><meta name='viewport' content='width=device-width,initial-scale=1,minimum-scale=1,maximum-scale=1,user-scalable=no'><style></style><script>function RFO(data){Android.dataLink(data);}</script></head><body><script>window.onload=function(){RFO('ready');}</script></body></html>"
...
FN.DEF GW_LOAD_PG(n, tot) % (internal) update GW lib loading progress
  IF tot>0 THEN HTML.LOAD.URL "javascript:document.getElementById('slider1').value='"+STR$(n/tot*100)+"'"
...
FN.DEF GW_CHECK_AND_DOWNLOAD(f$) % (internal) Download a single file to the device (except if it already exists)
  ...
  IF fe % already on device but at old place (not in 'GW' subfolder)
    ...
    FN.RTN 1
  END IF % else file is not on device -> download it from laughton or mougino servers
...
FN.DEF GW_CHECK_THEME(theme$) % (internal) Check existence on the device of main .css file for a theme
  ...
  IF !GW_DOWNLOAD_THEME(theme$)
    PRINT "There was a problem downloading GW theme '"+theme$+"'."
    END "Please check your connection and try again later."
  END IF
FN.END
IF !GW_SILENT_LOAD THEN GW_LOAD_PG(++GW_CFN, GW_NFN)
  ...
  IF page % setting transition needs an existing page
    ...
    ELSE % page under construction -> set transition via static html
      ...
      ELSE % DIALOG
        ...
        END IF % /transition of new/existing dialog
      END IF % /panel/dialog
    END IF % /page already-rendered/under-construction
  END IF % /last-edited-page exists
FN.END
IF !GW_SILENT_LOAD THEN GW_LOAD_PG(++GW_CFN, GW_NFN)
...
FN.DEF GW_CUSTO_DLGBTN(page, dlg, btn$, custo$)
  ...
  custo$=LOWER$(custo$)
  IF IS_IN("icon=", custo$) & 0=IS_IN("iconpos", custo$) ~
    & 0=IS_IN("notext", custo$) THEN custo$="iconpos=left "+custo$
...
FN.DEF GW_ADD_BAR_BTN$(ba$, type$) % (internal) for TITLEBAR and FOOTBAR (type$="left"|"right")
  ...
  IF 0=IS_IN("ui-btn-icon-", cs$) THEN cs$+=" ui-btn-icon-left"
  BUNDLE.GET 1, "gw-last-edited-page", page
  IF page THEN theme$=GW_THEME_OF_PAGE$(page)
  IF theme$="ios" | theme$="android-holo" | theme$="metro"
    e$="<a "+cs$+" "
...
FN.DEF GW_THEME_CUSTO$(ctl$) % (internal) Return a formated theme customization for this control family
  ...
  IF !tct % No theme customization -> return empty string
    IF ctl$="pgbar"
      FN.RTN "#5393c5,#6facd5" % except for progressbars: blue gradient by default
  ...
  ELSEIF ctl$="spinner" % spinner
    r$=REPLACE$(r$, "color=", "")
  ELSEIF ctl$="pgbar" % progressbar
    SW.BEGIN LOWER$(r$)
      SW.CASE "color=b" : r$ = "#4e9d4e,#5cb85c" : SW.BREAK % 'color=b': green gradient
      SW.CASE "color=c" : r$ = "#b94743,#d9534f" : SW.BREAK % 'color=c': red gradient
      SW.CASE "color=d" : r$ = "#cc9342,#f0ad4e" : SW.BREAK % 'color=d': orange gradient
      SW.CASE "color=e" : r$ = "#c21e63,#e57eaa" : SW.BREAK % 'color=e': pink gradient
      SW.CASE "color=f" : r$ = "#2b689c,#337ab7" : SW.BREAK % 'color=f': dark blue gradient
      SW.DEFAULT : r$ = "#5393c5,#6facd5" % 'color=a': blue gradient (by default)
    SW.END
...
FN.DEF GW_ADD_LOADING_IMG(page, img$)
  ! Check resource
  ...
  css$ += "background:url(" + img$ + ") center no-repeat #fff}"
  ! Declare JS
...
FN.DEF GW_ADD_PROGRESSBAR$(label$)
  t$="from("+REPLACE$(GW_THEME_CUSTO$("pgbar"), ",", "),to(")+")"
...
FN.DEF GW_ADD_IMAGE$(img$)
  ...
  IF i
    ...
    img$ = LEFT$(img$, i-1)
  END IF
  IF IS_APK() THEN MAKE_SURE_IS_ON_SD(img$) % in APK mode: if image is not on sdcard, copy it from assets
  ! Create the control
  u$=GW_NEWID$("image")
  IF LEN(lnk$)
...
FN.DEF GW_GET_IMAGE_DIM$(img$) % return dimension of image in the form "WxH"
  ...
  script$+="img.onload=function(){RFO(img.width+'x'+img.height);img=null;};"
  script$+="img.src="+CHR$(34)+img$+CHR$(34)+";"
  JS(script$)
...
FN.DEF GW_ADD_ICON(page, ico$, w, h)
  IF IS_APK() THEN MAKE_SURE_IS_ON_SD(ico$) % in APK mode: if icon is not on sdcard, copy it from assets
...
FN.DEF GW_ADD_VIDEO$(video$)
  ...
  IF IS_APK() & 0=IS_IN("http", video$) THEN MAKE_SURE_IS_ON_SD(video$)
...
FN.DEF GW_ADD_AUDIO$(audio$)
  ...
  IF IS_APK() & 0=IS_IN("http", audio$) THEN MAKE_SURE_IS_ON_SD(audio$)
...
FN.DEF GW_FORMAT_LINK$(link$) % (internal)
  ...
  ELSEIF LOWER$(LEFT$(link$, 4))="http" | LEFT$(link$,1)="#"
...
FN.DEF GW_ADD_SLIDER$(label$, n_min, n_max, n_step, n_ini)
  ...
  % e$="<div data-role='fieldcontain' "+t$+">"
  e$+="<label for='"+u$+"'>"+WEB$(label$)+"</label>"
  i=IS_IN("style='", t$)
  IF i % Customize <input> with jQuery custo but w/o CSS custo
    j=IS_IN("'", t$, i+7)
    t$=TRIM$(REPLACE$(t$, MID$(t$, i, j-i+1), ""))
  END IF
  e$+="<input name='"+u$+"' id='"+u$+"' "+t$
...
FN.DEF GW_MODIFY(ctl_id, key$, txt$)
  ...
  ELSEIF ctyp$="slider" % SLIDER / PROGRESSBAR
    IF key$="val"
      JS("document.getElementById("+DQ$+ctl$+DQ$+").value="+DQ$+txt$+DQ$)
    ELSEIF key$="min"
      JS("document.getElementById("+DQ$+ctl$+DQ$+").min="+DQ$+txt$+DQ$)
    ELSEIF key$="max"
      JS("document.getElementById("+DQ$+ctl$+DQ$+").max="+DQ$+txt$+DQ$)
    ELSEIF key$="step"
      JS("document.getElementById("+DQ$+ctl$+DQ$+").step="+DQ$+txt$+DQ$)
    ELSEIF key$="text"
      JS("$(\"label[for='"+ctl$+"']\").html(\""+WEB$(txt$)+"\")")
  ...
  ELSEIF key$="text" & (ctyp$="inputlist" | ctyp$="submit") % INPUTLIST, SUBMIT
    JS("document.getElementById("+DQ$+ctl$+DQ$+").value="+DQ$+WEB$(txt$)+DQ$)
  ELSEIF IS_IN("input", ctyp$)=1 | ctyp$="colorpicker" | ctyp$="inpbox" % ALL INPUT*, COLORPICKER
    ...
    ELSEIF key$="input"
      ...
      ELSE
        JS("document.getElementById("+DQP$(ctl$)+").value="+DQP$(txt$))
      END IF
    ELSE % Unknown key -> assume HTML attribute
      IF LEFT$(ctl$,1)<>"." THEN ctl$="#"+ctl$ % handle MODIFYing a whole class = multiple controls at once
      JS("$('"+ctl$+"').attr('"+key$+"',"+DQ$+txt$+DQ$+")")
  ...
  ELSEIF key$="content" & (ctyp$="image" | ctyp$="audio" | ctyp$="video") % IMAGE, AUDIO, VIDEO
    IF IS_APK() & 0=IS_IN("http", txt$) THEN MAKE_SURE_IS_ON_SD(txt$)
    i=IS_IN(">", txt$, -1) : IF i THEN poster$=MID$(txt$, i+1) : txt$=LEFT$(txt$, i-1)
    JS("document.getElementById("+DQ$+ctl$+DQ$+").src="+DQ$+txt$+DQ$)
    IF LEN(poster$) THEN JS("document.getElementById("+DQ$+ctl$+DQ$+").poster="+DQ$+poster$+DQ$)
...
IF !IS_APK()
  sys$="../../.gw.last.check" : GRABFILE v$, sys$
  TIME y$,m$,d$ : IF y$+m$+d$=v$ THEN GOTO newv_skip % already checked today > skip
  HTML.LOAD.URL "javascript:var e=document.getElementById('label1');e.innerHTML+='<small><i>Editor mode: checking GW lib update</i></small>'"
  ...
  BYTE.OPEN w, fid, sys$ : BYTE.WRITE.BUFFER fid, y$+m$+d$ : BYTE.CLOSE fid % update last time checked
  v$=LEFT$(v$, IS_IN("\n", v$)-1)
  ...
  GW_HIDE_SPINNER()
  IF VERSION$() > "01.90.01" THEN RUN ELSE GOTO newv_skip
  newv_err:
  GW_HIDE_SPINNER()

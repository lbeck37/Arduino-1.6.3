...
void Servo::write(int value)
{
    ...
    if (value < MIN_PULSE_WIDTH)
    {
        if (value < 0)
            ...
            value = 180;
#if 1
    		//Serial << "Servo::write(): Call MyMap()" << endl;
    		//Serial << "Servo::write(): Call map( " << value << ", " << 0 << ", " << 180 << ", " << this->min << ", " << this->max << ")"<< endl;

        value = map(value, 0, 180, this->min, this->max);
        //value = MyMap(value, 0, 180, this->min, this->max);
#else
    value= 1630;
#endif
    }
		//Serial << "Servo::write(): Call this->writeMicroseconds(" << value << ")" << endl;
    this->writeMicroseconds(value);
		//Serial << "Servo::write(): End" << endl;
}

void Servo::writeMicroseconds(int value)
{
		//Serial << "Servo::writeMicroseconds(): value " << value << endl;
    ...
    if ((this->servoChannel <= MAX_SERVOS) && (this->attached()))   // ensure channel is valid
    {
        if (value < this->min)          // ensure pulse width is valid
    		...
    		//Serial << "Servo::writeMicroseconds(): Call usToTicks()" << endl;
        value = usToTicks(value);  // convert to ticks
        this->ticks = value;
        // do the actual write
    		//Serial << "Servo::writeMicroseconds(): Call ledcWrite()" << endl;
